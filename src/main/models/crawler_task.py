...
class CrawlerTask(Base):
    ...
    login_enabled = Column(Boolean, nullable=False, default=False, comment="是否启用Playwright登录")
    login_username = Column(String(200), nullable=False, default="", comment="登录账号")
    login_password = Column(String(200), nullable=False, default="", comment="登录密码")
    playwright_login_url = Column(String(500), nullable=False, default="", comment="Playwright登录页URL")
    playwright_login_entry_xpath = Column(String(500), nullable=False, default="", comment="登录入口XPath")
    playwright_login_username_xpath = Column(String(500), nullable=False, default="", comment="登录用户名输入框XPath")
    playwright_login_password_xpath = Column(String(500), nullable=False, default="", comment="登录密码输入框XPath")
    playwright_login_submit_xpath = Column(String(500), nullable=False, default="", comment="登录提交按钮XPath")
    playwright_login_success_xpath = Column(String(500), nullable=False, default="", comment="登录成功判断XPath")
    playwright_login_timeout = Column(Integer, nullable=False, default=60, comment="登录超时秒数")
    playwright_headless = Column(Boolean, nullable=False, default=True, comment="Playwright无头模式")
    enable_content_image_placeholder = Column(Boolean, nullable=False, default=False, comment="是否启用正文图片占位")
    content_root_xpath = Column(JSONB, nullable=True, comment="正文根节点XPath")
    content_image_xpath = Column(JSONB, nullable=True, comment="正文图片XPath")
    content_image_placeholder_template = Column(String(200), nullable=False, default="![图片{index}]({url})", comment="图片占位模板")
    append_content_image_mapping = Column(Boolean, nullable=False, default=False, comment="是否追加图片映射")
    custom_methods = Column(JSONB, nullable=True, comment="自定义方法重写源码映射 {method_name: source_code}")
